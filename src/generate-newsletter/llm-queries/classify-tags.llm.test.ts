...
describe('ClassifyTags', () => {
  test('execute calls generateObject with correct params and returns object', async () => {
    ...
    expect(() =>
      callArg.schema.parse({ tag1: 'a', tag2: 'b', tag3: 'c' }),
    ).not.toThrow();
    expect(() => callArg.schema.parse({ tag1: 'a', tag2: 'b' })).toThrow();
